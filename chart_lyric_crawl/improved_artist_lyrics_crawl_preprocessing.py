...
browser.get("https://music.naver.com/search/search.nhn?query=%s" % (artist_name))
...
browser.get("https://music.naver.com/artist/track.nhn?artistId=%s&filteringOptions=RELEASE&sorting=newRelease" % (artist_id))
html = browser.page_source
...
for page in range(1, max_page+1, 1):
    browser.get("https://music.naver.com/artist/track.nhn?artistId=%s&filteringOptions=RELEASE&sorting=newRelease&page=%d" % (artist_id, page))
    html = browser.page_source
...
for t in trackId_list:
    ...
    browser.get("https://music.naver.com/lyric/index.nhn?trackId=%s" % t)
    html = browser.page_source

    title = BeautifulSoup(html, "lxml").find('span', class_='ico_play').text
    
    artist = str(BeautifulSoup(html, "lxml").find('span', class_='artist').find('a'))

    artist = str(re.findall(r'\>(.*?)\<', artist))[2:-2]

    lyrics = str(BeautifulSoup(html, "lxml").find('div', class_='show_lyrics'))
...
df_lyric_data.to_excel(writer, sheet_name='lyrics_data', startrow=1, header=True)
